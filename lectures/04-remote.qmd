...
[![](/img/zoom.png){fig-align="right"}](cccccc)
...
5.  **`tmux`** or **`screen`**: Both **`tmux`** and **`screen`** are terminal multiplexers, allowing you to run multiple terminal sessions within a single terminal window. They are especially useful in remote computing scenarios, as they enable you to keep processes running even after disconnecting from the remote machine. You can later reattach to the sessions and continue working. To start a new session, run **`tmux`** or **`screen`** on the remote machine. To reattach to a session after disconnecting, use **`tmux attach`** or **`screen -r`**.
