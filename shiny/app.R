...
server <- function(input, output) {
  ...
  data <- read.csv("../output/blast_annot_go.tab", sep = '\t', header = TRUE)
  
  # Define reactive expression to subset table based on user input
  subset_data <- reactive({
    subset(data, V3 == input$column_value) # Replace COLUMN_NAME with the name of the column you want to filter on
  })
